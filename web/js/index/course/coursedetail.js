 ...
 * 我的课程
 */
define(["/web/static/lib/js/jquery.raty.min.js", "Pagination"], function () {
    var classManage = new Vue({
        ...
        methods: {
            ...
            goLogin: function () {
                self.$modal.openDialog("/web/js/index/login/login-dialog.html", "用户登录");
            },
                    ...
                    if (result.resultCode == 0) {
                        ...
                        if (type == 4) {
                            ...
                            if (result.data.kszt == 'wccj') {
                                self.$modal.warn("考试还未出成绩，请等待！");
                            } else if (result.data.kszt == 'yccj') {
                                //已经考完出成绩了，跳转到成绩详情页
                                window.location.href = "/ucenter/view_exam?cjid=" + result.data.cjid + "&sjid=" + result.data.sjid;
                            } else if (result.data.kszt == 'wks') {
                                self.$modal.warn("当前考试还未开始！开始日期：" + date(result.data.kssj));
                            } else if (result.data.kszt == 'yjs') {
                                self.$modal.warn("当前考试已截止！截止日期：" + date(result.data.jssj));
                            } else {
                                window.location.href = '/ucenter/course/learning/' + courseId + '?chapterId=' + chapterId;
                            }
                        }
            ...
            joinCourse: function (url) {
                if (this.course.isNeedPwd) {
                    this.$modal.show(this, {
                        templateURL: __uri('/web/js/index/course/modal/modal_password.js'),//必填
                        surecb: function () {
                            location.href = url;
                        },//点击关闭时的回调，如没有则不填
                        title: "课程密码",//必填
        ...
        components: {
            'addnote': function (res, rej) {
                require([__uri('/web/js/index/course/createNote.js?v')], res);
            },
            'sc': function (res, rej) {
                require([__uri('/web/html/_layout_/sc/sc.js?v')], res);
            },
            'answer': function (res, rej) {
                require([__uri('/web/js/index/course/part/part_answer.js')], res);
            },
            'course_source': function (res, rej) {
                require([__uri('/web/js/index/course/part/part_source.js')], res);
            },
            'course_homework': function (res, rej) {
                require([__uri('/web/js/index/course/course_homework.js')], res);
            }
        },
        created: function () {
            this.getLoginStatus();
            ...
            // this.loadScore('halfstar',true);
            this.loadScore2();
            // this.loadRecommendCourse();
            // this.loadStudyStudents();
            // this.judgeCollect();
            // this.getMenuList();
        ...
        mounted: function () {
            // Utils.controlEleLen('content','showLen');
        }
    });
    return classManage;
});