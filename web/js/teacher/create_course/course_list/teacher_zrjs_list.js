...
mod.teacher_zrjs_list = Vue.component('teacher_zrjs_list', {
    template: __inline("/web/html/teacher/create_course/teacher_zrjs_list.html"),
    data: function () {
        return {
            jxdId: '',
            kcid: '',
    ...
    methods: {
        ...
        showConfirm: function (js) {
            var self = this;
            self.id = js.id;
            self.$modal.confirm('取消教师' + js.name + '在课程' + self.options.kc.kcMc + '中责任教师的身份？', self.submit, null);
        },
        submit: function () {
            var self = this;
            var data = {
