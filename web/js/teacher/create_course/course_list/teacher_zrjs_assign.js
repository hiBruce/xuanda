...
mod.teacher_zrjs_assign = Vue.component('teacher_zrjs_assign', {
    template: __inline("/web/html/teacher/create_course/teacher_zrjs_assign.html"),
    data: function () {
        return {
            pageSize: 10,
            pageNo: 1,
    ...
    methods: {
        ...
        tjGlJs: function (js) {
            var self = this;
            self.$modal.confirm('将教师' + js.name + '设置为课程' + self.options.kc.kcMc + '的责任教师？', function () {
                var data = {
                    kcId: self.options.kc.kcId,
                    jsIdStr: js.id,
                    jxdId: self.options.jxdInfo.id,
        ...
        scGlJs: function (js) {
            var self = this;
            self.$modal.confirm('取消教师' + js.name + '在课程' + self.options.kc.kcMc + '中责任教师的身份？', function () {
                var data = {
                    kcId: self.options.kc.kcId,
                    jsId: js.id,
                    jxdId: self.options.jxdInfo.id,
