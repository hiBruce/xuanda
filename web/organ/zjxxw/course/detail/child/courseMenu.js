define(function () {
    var courseMenu = Vue.component('courseMenu', {
        template: __inline("./courseMenu.html"),
        data: function () {
            return {
                rows_chapter: [],
                learningStatus:{},
                  ...
                  if(result.success){
                      ...
                      if(type==4){
                          ...
                          if(result.data.kszt=='wccj'){
                              errorMsg ="测试还未出成绩，请等待！";
                          }else if(result.data.kszt=='yccj'){
                              //已经考完出成绩了，跳转到成绩详情页
                              Url = "/ucenter/view_exam?cjid="+result.data.cjid+"&sjid="+result.data.sjid + "&courseId=" +result.data.courseId+ "&chapterId=" + result.data.chapterId+"&xs=1";
                          }/*else if(result.data.kszt=='wks'){
                              errorMsg ="当前测试还未开始！开始日期："+date(result.data.kssj);
                          }else if(result.data.kszt=='yjs'){
                              errorMsg ="当前测试已截止！截止日期："+date(result.data.jssj);
                          }*/
                      }
