...
define(['c3'], function (c3) {
    var xxjd = Vue.component('xxjd', {
                ...
                var chart = c3.generate({
                    ...
                    data: {
                        "columns": [
                            ...
                            ['测试', this.progress.ksFs],
                            ['练习', this.progress.lxFs],
                            ['文本', this.progress.wbFs],
                            ['互动', this.progress.tlFs],
                            ['未完成分数', 100 -parseFloat(this.progress.tlFs)- parseFloat(this.progress.spFs) - parseFloat(this.progress.zyFs) - parseFloat(this.progress.ksFs) - parseFloat(this.progress.lxFs) - parseFloat(this.progress.wbFs)]
                        ],
                        "type": "donut",
                        colors: {
                            "未完成分数": '#f4f4f4',
                    ...
                    donut: {
                        title: parseFloat(parseFloat(this.progress.spFs) + parseFloat(this.progress.zyFs) + parseFloat(this.progress.ksFs) + parseFloat(this.progress.lxFs) + parseFloat(this.progress.wbFs) + parseFloat(this.progress.tlFs)).toFixed(2) + "分",
                        width: 20,
                        label: {
                            show: false//隐藏图形上的进度文字
                        }
        ...
        created: function () {
            ...
            this.isFirst = this.$store.getters.getIsFirst();
            if (this.course.id) {
                this.getData()
            } else {
                var self = this;
