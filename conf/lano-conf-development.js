var oAppConfig = {
    ...
    'deploy': {
        /*开发模式下应用将要被部署的位置*/
        'real-path': 'D:/lano_node/lano-cloud-organ'
    },
    ...
    // 'lano-sso-client': {
    //     //单点登录的认证服务器uri
    //     'lano-sso-server-uri': 'http://localhost:3000/login'
    // },
    'redis': {
        'host': '10.100.23.83',
        'port': 6379,
    ...
    'lano-data-proxy': {
        'data-sources': {
            'default': {
                ...
                // 'urls': ['http://192.168.1.202:8080/service'],
                'default-path': '/',
                'pulse-path': '',
                'timeout': 300000
            },
    ...
    'lano-security': {
        // 中间件挂载路径， 默认为'/apps'
        'mount-path': ['/admin', '/teacher','/ucenter'],
        ...
        'validate-rules': [
            {
                'roleList': ['default'],
                'patternList': ['/admin/**', '/teacher/**','/ucenter/**']
            }
        ]
    },
    ...
    'upload': {
        ...
        'local': {
            // 文件服务器位置
            'fileServerUrl': 'http://localhost:3006/upload',
            'location': 'upload'
        }
    }
};
